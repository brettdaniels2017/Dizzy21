...
- **Land tools (per rock, reset on a newly unlocked land; old land keeps its own):** Chip crew, Mine carts, Bore drill, Powder charges, Orbital quarry.
- **Lands (paid unlock, not auto from total mined):** Pebble pit (free) → Granite → Quartz → Gold seam → Obsidian → Meteorite. Unlocking a land does **not** switch you onto it; tap the land card to change rocks.
...
- **Bore drill:** one placeable. First buy goes to the gear rack; drag onto the rock. Extra buys upgrade that same large drill (about 3× old size, color tiers like carts). Not multiple drills.
- Future placeables should follow the same one-tool, upgrade-in-place pattern (`PLACEABLES` array).
...
- Land **unlock** prices were raised **75%** (Granite 1400, Quartz 15750, Gold 148750, Obsidian 1.3125M, Meteorite 11.375M).
...
- Land income multiplier is on the land (`mult` 1 → 10). Idle of a land = (that land’s auto tools + placed drill) × land.mult.
...
- `syncMiners` — cart factory/rail/cart + crew mill/walkers
- `syncPlaceables` — gear rack + mounted drill
